	...
	class platform {
		...
		showReward(success, failure, com) {
			...
			if (!YYGGames.canShowReward()) {
				this.prompt0("No Available Video");
				com && com();
				// this.showNoVideo();
				return;
			}
			YYGGames.showReward({
				...
				rewardDismissed: () => {
					...
					this.prompt0("Pls watch the ad completely, so that you can claim your reward");
					// }
					// }
				}
			});
		...
		prompt(msg, duration) {
			if (!this.prompt_) {
				...
				this.prompt_.style.cssText =
					"overflow: hidden;word-break: break-all;word-wrap: break-word;font-family:siyuan;padding:10px 10px 10px 10px;min-height:40px;color: rgb(255, 255, 255);line-height: 50px;text-align:center;border-radius: 4px;position: fixed;top: 40%;left: 50%;transform: translate(-50%, -50%);z-index: 999999;background: rgba(0, 0, 0,.7);font-size: 20px;";
		...
		prompt0(msg, duration = 3000) {
			if (!Laya) {
				return;
			}
			if (!this.labelInfo) {
				this.labelInfo = new Laya.Label();
				this.labelInfo.align = "center";
				this.labelInfo.valign = "middle";
				this.labelInfo.bgColor = "#000000";
				this.labelInfo.color = "#ffffff";
				this.labelInfo.fontSize = 30
				this.labelInfo.centerX = 0;
				this.labelInfo.centerY = -50;
				this.labelInfo.zOrder = 100000000000;
			}
			this.labelInfo.width = null;
			this.labelInfo.height = null;
			this.labelInfo.wordWrap = false;
			this.labelInfo.visible = false;
			this.labelInfo.text = msg;
			this.labelInfo.height += 40
			if (this.labelInfo.width > Laya.stage.width - 100) {
				this.labelInfo.width = Laya.stage.width - 100;
				this.labelInfo.wordWrap = true;
			}

			this.labelInfo.visible = true;
			Laya.stage.addChild(this.labelInfo);
			Laya.timer.clear(this, this.clearLabel)
			Laya.timer.once(duration, this, this.clearLabel)
		}
		clearLabel() {
			this.labelInfo.visible = false;
		}
			...
			if (!this._box_adTwo) {
				...
				class box_adTwoScript extends Laya.Script {
					...
					onEnable() {
						...
						this.img_ad1.visible = this.img_ad1Ad.visible = this.owner.visible = false;
						this.img_ad0.alpha = 0.8;
						let adList = platform.getInstance().getForgames();
						if (!adList.length) {
							return;
						}
