    ...
    return function e(t, o, i) {
        ...
        5: [ function(t, e, o) {
            ...
            Broadphase.prototype.needBroadphaseCollision = function(t, e) {
                return 0 != (t.collisionFilterGroup & e.collisionFilterMask) && 0 != (e.collisionFilterGroup & t.collisionFilterMask) && (0 == (t.type & s) && t.sleepState !== i.SLEEPING || 0 == (e.type & s) && e.sleepState !== i.SLEEPING);
            ...
            Broadphase.prototype.makePairsUnique = function(t, e) {
                for (var o = l, i = h, n = c, r = t.length, s = 0; s !== r; s++) i[s] = t[s], n[s] = e[s];
                t.length = 0, e.length = 0;
                for (s = 0; s !== r; s++) {
                    var a = i[s].id, p = n[s].id;
                    o[u = a < p ? a + "," + p : p + "," + a] = s, o.keys.push(u);
                }
        ...
        6: [ function(t, e, o) {
            ...
            GridBroadphase.prototype.collisionPairs = function(t, e, o) {
                for (var i = t.numObjects(), n = t.bodies, a = this.aabbMax, l = this.aabbMin, h = this.nx, c = this.ny, p = this.nz, u = c * p, d = p, v = 1, y = a.x, f = a.y, m = a.z, w = l.x, x = l.y, g = l.z, b = h / (y - w), B = c / (f - x), S = p / (m - g), E = (y - w) / h, A = (f - x) / c, C = (m - g) / p, R = .5 * Math.sqrt(E * E + A * A + C * C), P = r.types, q = P.SPHERE, M = P.PLANE, V = (P.BOX, 
                ...
                function addBoxToBins(t, e, o, i, n, r, s) {
                    ...
                    l *= d, y *= v, f *= u, m *= d, E *= v;
                    for (var A = a *= u; A <= f; A += u) for (var C = l; C <= m; C += d) for (var R = y; R <= E; R += v) {
                        var P = A + C + R;
                        V[P][z[P]++] = s;
                    }
                }
                for (N = 0; N !== i; N++) {
                    ...
                    switch (W.type) {
                      case q:
                        var I = et.position.x, L = et.position.y, j = et.position.z, O = W.radius;
                        addBoxToBins(I - O, L - O, j - O, I + O, L + O, j + O, et);
                        break;

                      case M:
                        W.worldNormalNeedsUpdate && W.computeWorldNormal(et.quaternion);
                        var H = W.worldNormal, k = w + .5 * E - et.position.x, _ = x + .5 * A - et.position.y, U = g + .5 * C - et.position.z, D = s;
                        D.set(k, _, U);
                        for (var Q = 0, G = 0; Q !== h; Q++, G += u, D.y = _, D.x += E) for (var X = 0, Y = 0; X !== c; X++, 
                        Y += d, D.z = U, D.y += A) for (var Z = 0, K = 0; Z !== p; Z++, K += v, D.z += C) if (D.dot(H) < R) {
                            var J = G + Y + K;
                            V[J][z[J]++] = et;
                        }
                        break;
        ...
        9: [ function(t, e, o) {
            ...
            Ray.prototype.intersectConvex = function(t, e, o, i, n) {
                for (var r = B, s = S, a = n && n.faceList || null, l = t.faces, h = t.vertices, c = t.faceNormals, p = this._direction, u = this.from, d = this.to, v = u.distanceTo(d), x = a ? a.length : l.length, g = this.result, b = 0; !g._shouldStop && b < x; b++) {
                    var E = a ? a[b] : b, A = l[E], C = c[E], R = e, P = o;
                    s.copy(h[A[0]]), R.vmult(s, s), s.vadd(P, s), s.vsub(u, s), R.vmult(C, r);
                    var q = p.dot(r);
                    if (!(Math.abs(q) < this.precision)) {
                        var M = r.dot(s) / q;
                        if (!(M < 0)) {
                            p.mult(M, y), y.vadd(u, y), f.copy(h[A[0]]), R.vmult(f, f), P.vadd(f, f);
                            for (var V = 1; !g._shouldStop && V < A.length - 1; V++) {
                                m.copy(h[A[V]]), w.copy(h[A[V + 1]]), R.vmult(m, m), R.vmult(w, w), P.vadd(m, m), 
                                P.vadd(w, w);
            ...
            var E = new i(), A = new i(), C = new i(), R = new i(), P = new i(), q = new i(), M = (new l(), 
            []), V = new r();
            Ray.prototype.intersectTrimesh = function(t, e, o, i, n) {
                var s = E, a = M, l = V, h = S, c = A, p = C, u = R, d = q, v = P, x = (n && n.faceList, 
                ...
                for (var T = 0, N = a.length; !this.result._shouldStop && T !== N; T++) {
                    ...
                    var W = c.dot(s), I = s.dot(h) / W;
                    if (!(I < 0)) {
                        c.scale(I, y), y.vadd(p, y), t.getVertex(x[3 * F + 1], m), t.getVertex(x[3 * F + 2], w);
        ...
        32: [ function(t, e, o) {
            ...
            RaycastVehicle.prototype.updateFriction = function(t) {
                for (var e = d, o = this.wheelInfos, n = o.length, r = this.chassisBody, s = y, a = v, l = 0; l < n; l++) {
                    (f = (A = o[l]).raycastResult.body) && 0, A.sideImpulse = 0, A.forwardImpulse = 0, 
                    s[l] || (s[l] = new i()), a[l] || (a[l] = new i());
                }
                for (l = 0; l < n; l++) {
                    if (f = (A = o[l]).raycastResult.body) {
                        var h = a[l];
                        this.getWheelTransformWorld(l).vectorToWorldFrame(u[this.indexRightAxis], h);
                        var c = A.raycastResult.hitNormalWorld, p = h.dot(c);
                        c.scale(p, e), h.vsub(e, h), h.normalize(), c.cross(h, s[l]), s[l].normalize(), 
                        A.sideImpulse = resolveSingleBilateral(r, A.raycastResult.hitPointWorld, f, A.raycastResult.hitPointWorld, h), 
                        A.sideImpulse *= 1;
                    }
                }
                this.sliding = !1;
                for (l = 0; l < n; l++) {
                    var f = (A = o[l]).raycastResult.body, m = 0;
                    if (A.slipInfo = 1, f) {
                        var w = A.brake ? A.brake : 0;
                        m = calcRollingFriction(r, f, A.raycastResult.hitPointWorld, s[l], w);
                        var x = w / (m += A.engineForce * t);
                        A.slipInfo *= x;
                    }
                    if (A.forwardImpulse = 0, A.skidInfo = 1, f) {
                        A.skidInfo = 1;
                        var g = A.suspensionForce * t * A.frictionSlip, b = g * g;
                        A.forwardImpulse = m;
                        var B = .5 * A.forwardImpulse, S = 1 * A.sideImpulse, E = B * B + S * S;
                        if (A.sliding = !1, E > b) {
                            this.sliding = !0, A.sliding = !0;
                            x = g / Math.sqrt(E);
                            A.skidInfo *= x;
                        }
                    }
                ...
                if (this.sliding) for (l = 0; l < n; l++) {
                    0 !== (A = o[l]).sideImpulse && A.skidInfo < 1 && (A.forwardImpulse *= A.skidInfo, 
                    A.sideImpulse *= A.skidInfo);
                }
                for (l = 0; l < n; l++) {
                    var A = o[l], C = new i();
                    if (C.copy(A.raycastResult.hitPointWorld), 0 !== A.forwardImpulse) {
                        var R = new i();
                        s[l].scale(A.forwardImpulse, R), r.applyImpulse(R, C);
                    }
                    if (0 !== A.sideImpulse) {
                        f = A.raycastResult.body;
                        var P = new i();
                        P.copy(A.raycastResult.hitPointWorld);
                        var q = new i();
                        a[l].scale(A.sideImpulse, q), r.pointToLocalFrame(C, C), C["xyz"[this.indexUpAxis]] *= A.rollInfluence, 
                        r.pointToWorldFrame(C, C), r.applyImpulse(q, C), q.scale(-1, q), f.applyImpulse(q, P);
                    }
                }
            };
        ...
        34: [ function(t, e, o) {
            ...
            SPHSystem.prototype.update = function() {
                for (var t = this.particles.length, e = r, o = this.speedOfSound, i = this.eps, n = 0; n !== t; n++) {
                    ...
                    (A = this.neighbors[n]).length = 0, this.getNeighbors(p, A), A.push(this.particles[n]);
                    for (var u = A.length, d = 0, v = 0; v !== u; v++) {
                        p.position.vsub(A[v].position, e);
                ...
                for (n = 0; n !== t; n++) {
                    ...
                    var A;
                    for (u = (A = this.neighbors[n]).length, v = 0; v !== u; v++) {
                        var C = A[v];
                        E.position.vsub(C.position, g);
                        var R = g.norm();
                        B = -C.mass * (this.pressures[n] / (this.densities[n] * this.densities[n] + i) + this.pressures[v] / (this.densities[v] * this.densities[v] + i)), 
                        this.gradw(g, x), x.mult(B, x), m.vadd(x, m), C.velocity.vsub(E.velocity, b), b.mult(1 / (1e-4 + this.densities[n] * this.densities[v]) * this.viscosity * C.mass, b), 
        ...
        35: [ function(t, e, o) {
            ...
            Spring.prototype.applyForce = function() {
                var t = this.stiffness, e = this.damping, o = this.restLength, i = this.bodyA, y = this.bodyB, f = n, m = r, w = s, x = a, g = v, b = l, B = h, S = c, E = p, A = u, C = d;
                ...
                i.force.vsub(x, i.force), y.force.vadd(x, y.force), S.cross(x, A), E.cross(x, C), 
                i.torque.vsub(A, i.torque), y.torque.vadd(C, y.torque);
            };
        }, {
            "../math/Vec3": 30
        } ],
        ...
        38: [ function(t, e, o) {
            ...
            ConvexPolyhedron.prototype.findSeparatingAxis = function(t, e, o, i, n, r, s, a) {
                ...
                if (this.uniqueAxes) for (B = 0; B !== this.uniqueAxes.length; B++) {
                    if (o.vmult(this.uniqueAxes[B], l), !1 === (A = this.testSepAxis(l, t, e, o, i, n))) return !1;
                    A < g && (g = A, r.copy(l));
                } else for (var b = s ? s.length : this.faces.length, B = 0; B < b; B++) {
                    var S = s ? s[B] : B;
                    if (l.copy(this.faceNormals[S]), o.vmult(l, l), !1 === (A = this.testSepAxis(l, t, e, o, i, n))) return !1;
                    A < g && (g = A, r.copy(l));
                }
                if (t.uniqueAxes) for (B = 0; B !== t.uniqueAxes.length; B++) {
                    if (n.vmult(t.uniqueAxes[B], h), 0, !1 === (A = this.testSepAxis(h, t, e, o, i, n))) return !1;
                    A < g && (g = A, r.copy(h));
                } else for (var E = a ? a.length : t.faces.length, B = 0; B < E; B++) {
                    var A;
                    S = a ? a[B] : B;
                    if (h.copy(t.faceNormals[S]), n.vmult(h, h), 0, !1 === (A = this.testSepAxis(h, t, e, o, i, n))) return !1;
                    A < g && (g = A, r.copy(h));
                }
            ...
            var g = new n(), b = new n(), B = new n(), S = new n(), E = new n(), A = new n(), C = new n(), R = new n();
            ConvexPolyhedron.prototype.clipFaceAgainstHull = function(t, e, o, i, n, r, s) {
                for (var a = g, l = b, h = B, c = S, p = E, u = A, d = C, v = R, y = i, f = [], m = -1, w = Number.MAX_VALUE, x = 0; x < this.faces.length; x++) {
                ...
                if (!(m < 0)) {
                    var q = this.faces[m];
                    q.connectedFaces = [];
                    for (var M = 0; M < this.faces.length; M++) for (var V = 0; V < this.faces[M].length; V++) -1 !== q.indexOf(this.faces[M][V]) && M !== m && -1 === q.connectedFaces.indexOf(M) && q.connectedFaces.push(M);
                    y.length;
                    for (var z = q.length, T = 0; T < z; T++) {
                        var N = this.vertices[q[T]], F = this.vertices[q[(T + 1) % z]];
                        ...
                        e.vadd(u, u);
                        u.dot(p);
                        var W = q.connectedFaces[T];
                        d.copy(this.faceNormals[W]);
                        var I = this.getPlaneConstantOfFace(W);
                    ...
                    v.copy(d), o.vmult(v, v);
                    for (L = I - v.dot(e), M = 0; M < y.length; M++) {
                        var j = v.dot(y[M]) + L;
            ...
            var q = new n(), M = new n(), V = new n();
            ConvexPolyhedron.prototype.pointIsInside = function(t) {
                var e = this.vertices.length, o = this.vertices, i = this.faces, n = this.faceNormals, r = this.faces.length, s = q;
        ...
        46: [ function(t, e, o) {
            ...
            GSSolver.prototype.solve = function(t, e) {
                ...
                if (0 !== v) {
                    for (w = 0; w !== f; w++) {
                        var S = (C = y[w]).vlambda, E = C.wlambda;
                    ...
                    for (c = 0; c !== p; c++) {
                        ...
                        for (var A = 0; A !== v; A++) {
                            B = d[A];
                            o = g[A], i = x[A], (h = b[A]) + (a = i * (o - B.computeGWlambda() - B.eps * h)) < B.minForce ? a = B.minForce - h : h + a > B.maxForce && (a = B.maxForce - h), 
                            b[A] += a, l += a > 0 ? a : -a, B.addToWlambda(a);
                        }
                        if (l * l < u) break;
                    }
                    for (w = 0; w !== f; w++) {
                        var C, R = (C = y[w]).velocity, P = C.angularVelocity;
                        R.vadd(C.vlambda, R), P && P.vadd(C.wlambda, P);
                    }
                }
                return c;
            };
        ...
        55: [ function(t, e, o) {
            ...
            Narrowphase.prototype.getContacts = function(t, e, o, i, n, r, s) {
                ...
                for (var a = m, l = w, h = y, c = f, p = 0, u = t.length; p !== u; p++) {
                    var d = t[p], v = e[p], x = null;
                    d.material && v.material && (x = o.getContactMaterial(d.material, v.material) || null);
                    for (var g = 0; g < d.shapes.length; g++) {
                        ...
                        for (var b = d.shapes[g], B = 0; B < v.shapes.length; B++) {
                            ...
                            if (!(h.distanceTo(c) > b.boundingSphereRadius + S.boundingSphereRadius)) {
                                var E = null;
                                b.material && S.material && (E = o.getContactMaterial(b.material, S.material) || null), 
                                this.currentContactMaterial = E || x || o.defaultContactMaterial;
                                var A = this[b.type | S.type];
                                A && (b.type < S.type ? A.call(this, b, S, h, c, a, l, d, v, b, S) : A.call(this, S, b, c, h, l, a, v, d, b, S));
                            }
                        }
                    }
                }
            ...
            var B = new s(), S = new s(), E = (new s(), new s()), A = new s(), C = new s(), R = new s(), P = new s(), q = new s(), M = new s(), V = new s(), z = new s(), T = new s(), N = new s(), F = new i(), W = [];
            Narrowphase.prototype[n.types.SPHERE | n.types.TRIMESH] = Narrowphase.prototype.sphereTrimesh = function(t, e, o, i, n, s, l, h) {
                var c = C, p = R, u = P, d = q, v = M, y = V, f = F, m = A, w = S, x = W;
                a.pointToLocalFrame(i, s, o, v);
                var g = t.radius;
                f.lowerBound.set(v.x - g, v.y - g, v.z - g), f.upperBound.set(v.x + g, v.y + g, v.z + g), 
                e.getTrianglesInAABB(f, x);
                for (var b = E, I = t.radius * t.radius, L = 0; L < x.length; L++) for (var j = 0; j < 3; j++) {
                    if (e.getVertex(e.indices[3 * x[L] + j], b), b.vsub(v, w), w.norm2() <= I) m.copy(b), 
            ...
            Narrowphase.prototype[n.types.SPHERE | n.types.PLANE] = Narrowphase.prototype.spherePlane = function(t, e, o, i, n, r, s, a) {
                ...
                l.ni.mult(t.radius, l.ri), o.vsub(i, I), l.ni.mult(l.ni.dot(I), L), I.vsub(L, l.rj), 
                -I.dot(l.ni) <= t.radius) {
                    var h = l.ri, c = l.rj;
            ...
            Narrowphase.prototype[n.types.SPHERE | n.types.BOX] = Narrowphase.prototype.sphereBox = function(t, e, o, i, n, r, s, a) {
                ...
                for (var c = t.radius, p = !1, u = X, d = Y, v = Z, y = null, f = 0, m = 0, w = 0, x = null, g = 0, b = h.length; g !== b && !1 === p; g++) {
                    ...
                    if (E < S + c && E > 0) {
                        var A = U, C = D;
                        A.copy(h[(g + 1) % 3]), C.copy(h[(g + 2) % 3]);
                        var R = A.norm(), P = C.norm();
                        A.normalize(), C.normalize();
                        var q = k.dot(A), M = k.dot(C);
                        if (q < R && q > -R && M < P && M > -P) {
                            var V = Math.abs(E - S - c);
                            (null === x || V < x) && (x = V, m = q, w = M, y = S, u.copy(B), d.copy(A), v.copy(C), 
                            f++);
                        }
                    }
                }
                ...
                for (var T = l.get(), N = G, F = 0; 2 !== F && !p; F++) for (var W = 0; 2 !== W && !p; W++) for (var I = 0; 2 !== I && !p; I++) {
                    if (T.set(0, 0, 0), F ? T.vadd(h[0], T) : T.vsub(h[0], T), W ? T.vadd(h[1], T) : T.vsub(h[1], T), 
                    I ? T.vadd(h[2], T) : T.vsub(h[2], T), i.vadd(T, N), N.vsub(o, N), N.norm2() < c * c) p = !0, 
                ...
                for (F = 0; F !== H && !p; F++) for (W = 0; W !== H && !p; W++) if (F % 3 != W % 3) {
                    h[W].cross(h[F], L), L.normalize(), h[F].vadd(h[W], j), z.copy(o), z.vsub(j, z), 
                    z.vsub(i, z);
                    var K = z.dot(L);
                    L.mult(K, O);
                    for (I = 0; I === F % 3 || I === W % 3; ) I++;
            ...
            Narrowphase.prototype[n.types.SPHERE | n.types.CONVEXPOLYHEDRON] = Narrowphase.prototype.sphereConvex = function(t, e, o, i, n, r, s, a) {
                ...
                for (var m = !1, w = (d = 0, c.length); d !== w && !1 === m; d++) {
                    ...
                    S.vsub(B, E);
                    var A = E.dot(b), C = st;
                    if (o.vsub(B, C), A < 0 && C.dot(b) > 0) {
                        for (var R = [], P = 0, q = g.length; P !== q; P++) {
                            var M = l.get();
                            r.vmult(p[g[P]], M), i.vadd(M, M), R.push(M);
                        }
                        ...
                        for (P = 0; P !== g.length; P++) {
                            var F = l.get(), W = l.get();
                            r.vmult(p[g[(P + 1) % g.length]], F), r.vmult(p[g[(P + 2) % g.length]], W), i.vadd(F, F), 
                            i.vadd(W, W);
                            var I = J;
                            W.vsub(F, I);
                            var L = $;
                            ...
                            var k = l.get();
                            if (j.vsub(o, k), H > 0 && H * H < I.norm2() && k.norm2() < u * u) {
                                ...
                                return l.release(F), l.release(W), l.release(j), l.release(k), void l.release(O);
                            }
                            l.release(F), l.release(W), l.release(j), l.release(k), l.release(O);
                        }
                        for (P = 0, N = R.length; P !== N; P++) l.release(R[P]);
                    }
                }
            ...
            Narrowphase.prototype[n.types.SPHERE | n.types.HEIGHTFIELD] = Narrowphase.prototype.sphereHeightfield = function(t, e, o, i, n, r, s, l) {
                var h = e.data, c = t.radius, p = e.elementSize, u = Ct, d = At;
        ...
        56: [ function(t, e, o) {
            ...
            function World() {
                l.apply(this), this.dt = -1, this.allowSleep = !1, this.contacts = [], this.frictionEquations = [], 
                this.quatNormalizeSkip = 0, this.quatNormalizeFast = !1, this.time = 0, this.stepnumber = 0, 
                this.default_dt = 1 / 60, this.nextId = 0, this.gravity = new n(), this.broadphase = new m(), 
                this.bodies = [], this.solver = new s(), this.constraints = [], this.narrowphase = new a(this), 
                this.collisionMatrix = new h(), this.collisionMatrixPrevious = new h(), this.materials = [], 
                this.contactmaterials = [], this.contactMaterialTable = new d(), this.defaultMaterial = new c("default"), 
            ...
            var b = {
                ...
                body: null,
                contact: null
            }, E = [], A = [], C = [], R = [], P = (new n(), new n(), new n(), new n(), new n(), 
            new n(), new n(), new n(), new n(), new r(), new r()), q = new r(), M = new n();
            World.prototype.internalStep = function(t) {
                this.dt = t;
                var e, o = this.contacts, n = C, r = R, s = this.numObjects(), a = this.bodies, l = this.solver, h = this.gravity, c = this.doProfiling, p = this.profile, d = u.DYNAMIC, v = this.constraints, y = A, f = (h.norm(), 
                h.x), m = h.y, w = h.z, x = 0;
                for (c && (e = performance.now()), x = 0; x !== s; x++) {
                    if ((O = a[x]).type & d) {
                        var g = O.force, V = O.mass;
                        g.x += V * f, g.y += V * m, g.z += V * w;
                    }
                }
                x = 0;
                for (var z = this.subsystems.length; x !== z; x++) this.subsystems[x].update();
                c && (e = performance.now()), n.length = 0, r.length = 0, this.broadphase.collisionPairs(this, n, r), 
                c && (p.broadphase = performance.now() - e);
                var T = v.length;
                for (x = 0; x !== T; x++) {
                    if (!(k = v[x]).collideConnected) for (var N = n.length - 1; N >= 0; N -= 1) (k.bodyA === n[N] && k.bodyB === r[N] || k.bodyB === n[N] && k.bodyA === r[N]) && (n.splice(N, 1), 
                    r.splice(N, 1));
                }
                this.collisionMatrixTick(), c && (e = performance.now());
                var F = E, W = o.length;
                for (x = 0; x !== W; x++) F.push(o[x]);
                o.length = 0;
                var I = this.frictionEquations.length;
                for (x = 0; x !== I; x++) y.push(this.frictionEquations[x]);
                this.frictionEquations.length = 0, this.narrowphase.getContacts(n, r, this, o, F, this.frictionEquations, y), 
                c && (p.narrowphase = performance.now() - e), c && (e = performance.now());
                for (x = 0; x < this.frictionEquations.length; x++) l.addEquation(this.frictionEquations[x]);
                for (var L = o.length, j = 0; j !== L; j++) {
                    var O = (k = o[j]).bi, H = k.bj;
                    k.si, k.sj, (O.material && H.material && this.getContactMaterial(O.material, H.material) || this.defaultContactMaterial).friction;
                    if (O.material && H.material && (O.material.friction >= 0 && H.material.friction >= 0 && O.material.friction * H.material.friction, 
                    O.material.restitution >= 0 && H.material.restitution >= 0 && (k.restitution = O.material.restitution * H.material.restitution)), 
                    l.addEquation(k), O.allowSleep && O.type === u.DYNAMIC && O.sleepState === u.SLEEPING && H.sleepState === u.AWAKE && H.type !== u.STATIC) H.velocity.norm2() + H.angularVelocity.norm2() >= 2 * Math.pow(H.sleepSpeedLimit, 2) && (O._wakeUpAfterNarrowphase = !0);
                    if (H.allowSleep && H.type === u.DYNAMIC && H.sleepState === u.SLEEPING && O.sleepState === u.AWAKE && O.type !== u.STATIC) O.velocity.norm2() + O.angularVelocity.norm2() >= 2 * Math.pow(O.sleepSpeedLimit, 2) && (H._wakeUpAfterNarrowphase = !0);
                    this.collisionMatrix.set(O, H, !0), this.collisionMatrixPrevious.get(O, H) || (S.body = H, 
                    S.contact = k, O.dispatchEvent(S), S.body = O, H.dispatchEvent(S));
                }
                for (c && (p.makeContactConstraints = performance.now() - e, e = performance.now()), 
                x = 0; x !== s; x++) {
                ...
                for (x = 0; x !== T; x++) {
                    ...
                    (k = v[x]).update();
                    N = 0;
                    for (var _ = k.equations.length; N !== _; N++) {
                        var U = k.equations[N];
                        l.addEquation(U);
                    }
                }
                l.solve(t, this), c && (p.solve = performance.now() - e), l.removeAllEquations();
                var D = Math.pow;
                for (x = 0; x !== s; x++) {
                    if ((O = a[x]).type & d) {
                        var Q = D(1 - O.linearDamping, t), G = O.velocity;
                        G.mult(Q, G);
                        var X = O.angularVelocity;
                        if (X) {
                            var Y = D(1 - O.angularDamping, t);
                            X.mult(Y, X);
                        }
                    }
                }
                for (this.dispatchEvent(B), x = 0; x !== s; x++) {
                    (O = a[x]).preStep && O.preStep.call(O);
                }
                c && (e = performance.now());
                var Z = P, K = q, J = this.stepnumber, $ = u.DYNAMIC | u.KINEMATIC, tt = J % (this.quatNormalizeSkip + 1) == 0, et = this.quatNormalizeFast, ot = .5 * t;
                i.types.PLANE, i.types.CONVEXPOLYHEDRON;
                for (x = 0; x !== s; x++) {
                    var it = a[x], nt = it.force, rt = it.torque;
                    if (it.type & $ && it.sleepState !== u.SLEEPING) {
                        var st = it.velocity, at = it.angularVelocity, lt = it.position, ht = it.quaternion, ct = it.invMass, pt = it.invInertiaWorld;
                        st.x += nt.x * ct * t, st.y += nt.y * ct * t, st.z += nt.z * ct * t, it.angularVelocity && (pt.vmult(rt, M), 
                        M.mult(t, M), M.vadd(at, at)), lt.x += st.x * t, lt.y += st.y * t, lt.z += st.z * t, 
                        it.angularVelocity && (Z.set(at.x, at.y, at.z, 0), Z.mult(ht, K), ht.x += ot * K.x, 
                        ht.y += ot * K.y, ht.z += ot * K.z, ht.w += ot * K.w, tt && (et ? ht.normalizeFast() : ht.normalize())), 
                        it.aabb && (it.aabbNeedsUpdate = !0), it.updateInertiaWorld && it.updateInertiaWorld();
                    }
                }
                for (this.clearForces(), this.broadphase.dirty = !0, c && (p.integrate = performance.now() - e), 
                this.time += t, this.stepnumber += 1, this.dispatchEvent(b), x = 0; x !== s; x++) {
                    var ut = (O = a[x]).postStep;
                    ut && ut.call(O);
                }
                if (this.allowSleep) for (x = 0; x !== s; x++) a[x].sleepTick(this.time);
