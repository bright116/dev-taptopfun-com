        ...
        try {
            ...
            if (p < 0) {
                if (!e.substr(d).match(/^\s*$/)) {
                    var m = r.doc, N = m.createTextNode(e.substr(d));
            ...
            switch (p > d && appendText(p), e.charAt(p + 1)) {
              case "/":
                var E = e.indexOf(">", p + 3), h = e.substring(p + 2, E), f = u.pop();
                E < 0 ? (h = e.substring(p + 2).replace(/[\s<].*/, ""), i.error("end tag name: " + h + " is not complete:" + f.tagName), 
                E = p + 1 + h.length) : h.match(/\s</) && (h = h.replace(/[\s<].*/, ""), i.error("end tag name: " + h + " maybe not complete"), 
                E = p + 1 + h.length);
                var _ = f.localNSMap, T = f.tagName == h;
                if (T || f.tagName && f.tagName.toLowerCase() == h.toLowerCase()) {
                    if (r.endElement(f.uri, f.localName, h), _) for (var g in _) r.endPrefixMapping(g);
              ...
              default:
                c && position(p);
                var D = new ElementAttributes(), S = u[u.length - 1].currentNSMap, A = (E = parseElementStartPart(e, p, D, S, entityReplacer, i), 
                D.length);
                if (!D.closed && fixSelfClosed(e, E, D.tagName, l) && (D.closed = !0, n.nbsp || i.warning("unclosed xml attribute")), 
                c && A) {
...
function parseElementStartPart(e, t, n, r, i, o) {
    for (var a, s = ++t, c = S_TAG; ;) {
        ...
        switch (u) {
          case "=":
            if (c === S_ATTR) a = e.slice(t, s), c = S_EQ; else {
          ...
          case '"':
            if (c === S_EQ || c === S_ATTR) {
                if (c === S_ATTR && (o.warning('attribute value must after "="'), a = e.slice(t, s)), 
                t = s + 1, !((s = e.indexOf(u, t)) > 0)) throw new Error("attribute value no end '" + u + "' match");
                ...
                l = e.slice(t, s).replace(/&#?\w+;/g, i), n.add(a, l, t), o.warning('attribute "' + a + '" missed start quot(' + u + ")!!"), 
                t = s + 1, c = S_ATTR_END;
            }
            break;
          ...
          case "":
            return o.error("unexpected end of input"), c == S_TAG && n.setTagName(e.slice(t, s)), 
            s;
          ...
          case ">":
            switch (c) {
              ...
              case S_ATTR_SPACE:
                c === S_ATTR_SPACE && (l = a), c == S_ATTR_NOQUOT_VALUE ? (o.warning('attribute "' + l + '" missed quot(")!!'), 
    ...
    switch (e.charAt(t + 2)) {
      ...
      default:
        ...
        if (a > 1 && /!doctype/i.test(o[0][0])) {
            var s = o[1][0], c = a > 3 && /^public$/i.test(o[2][0]) && o[3][0], u = a > 4 && o[4][0], l = o[a - 1];
...
SaxO = {}, XMLReader.prototype = {
    ...
    for (var r = e.tagName, i = null, o = e.length; o--; ) {
        var a = e[o], s = a.qName, c = a.value;
        if ((p = s.indexOf(":")) > 0) var u = a.prefix = s.slice(0, p), l = s.slice(p + 1), d = "xmlns" === u && l; else l = s, 
        u = null, d = "xmlns" === s && "";
        a.localName = l, !1 !== d && (null == i && (i = {}, _copy(n, n = {})), n[d] = i[d] = c, 
        a.uri = "http://www.w3.org/2000/xmlns/", t.startPrefixMapping(d, c));
    }
    for (o = e.length; o--; ) {
    ...
    var p;
    (p = r.indexOf(":")) > 0 ? (u = e.prefix = r.slice(0, p), l = e.localName = r.slice(p + 1)) : (u = null, 
    l = e.localName = r);
    var m = e.uri = n[u || ""];
    if (t.startElement(m, l, r, e), !e.closed) return e.currentNSMap = n, e.localNSMap = i, 
    !0;
    if (t.endElement(m, l, r), i) for (u in i) t.endPrefixMapping(u);
...
function serializeToString(e, t, n, r, i) {
    ...
    switch (e.nodeType) {
      case ELEMENT_NODE:
        i || (i = []);
        i.length;
        var o = e.attributes, a = o.length, s = e.firstChild, c = e.tagName;
        ...
        for (u = 0; u < a; u++) {
            ...
            if (needNamespaceDefine(l = o.item(u), n, i)) {
                var d = l.prefix || "", p = l.namespaceURI, m = d ? " xmlns:" + d : " xmlns";
                t.push(m, '="', p, '"'), i.push({
                    prefix: d,
                    namespace: p
                });
        ...
        if (s || n && !/^(?:meta|link|img|br|hr|input)$/i.test(c)) {
            if (t.push(">"), n && /^script$/i.test(c)) for (;s; ) s.data ? t.push(s.data) : serializeToString(s, t, n, r, i), 
      ...
      case DOCUMENT_TYPE_NODE:
        var N = e.publicId, E = e.systemId;
        if (t.push("<!DOCTYPE ", e.name), N) t.push(' PUBLIC "', N), E && "." != E && t.push('" "', E), 
        t.push('">'); else if (E && "." != E) t.push(' SYSTEM "', E, '">'); else {
...
function importNode(e, t, n) {
    ...
    if (r || (r = t.cloneNode(!1)), r.ownerDocument = e, r.parentNode = null, n) for (var i = t.firstChild; i; ) r.appendChild(importNode(e, i, n)), 
    i = i.nextSibling;
    return r;
}
...
function cloneNode(e, t, n) {
    ...
    switch (t.childNodes && (r.childNodes = new NodeList()), r.ownerDocument = e, r.nodeType) {
      case ELEMENT_NODE:
        var a = t.attributes, s = r.attributes = new NamedNodeMap(), c = a.length;
        s._ownerElement = r;
    ...
    if (n) for (var l = t.firstChild; l; ) r.appendChild(cloneNode(e, l, n)), l = l.nextSibling;
    return r;
}
...
exports.XMLSerializer = XMLSerializer), DOMParser.prototype.parseFromString = function(e, t) {
    var n = this.options, r = new XMLReader(), i = n.domBuilder || new DOMHandler(), o = n.errorHandler, a = n.locator, s = n.xmlns || {}, c = /\/x?html?$/.test(t), u = c ? htmlEntity.entityMap : {
        lt: "<",
        gt: ">",
        amp: "&",
        quot: '"',
    ...
    r.domBuilder = n.domBuilder || i, c && (s[""] = "http://www.w3.org/1999/xhtml"), 
    s.xml = s.xml || "http://www.w3.org/XML/1998/namespace", e ? r.parse(e, s, u) : r.errorHandler.error("invalid doc source"), 
    i.doc;
    ...
    startElement: function(e, t, n, r) {
        ...
        for (var s = 0; s < a; s++) {
            e = r.getURI(s);
            var c = r.getValue(s), u = (n = r.getQName(s), i.createAttributeNS(e, n));
