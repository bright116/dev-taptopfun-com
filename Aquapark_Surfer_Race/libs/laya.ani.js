            ...
            return _createClass(BezierLerp, null, [{
                ...
                value: function(t, e, i, a, r) {
                    ...
                    for (l = o.length, h = 0; h < l; h += 2)
                        if (t <= o[h]) return BezierLerp._bezierResultCache[n] = o[h + 1],
                            o[h + 1];
            ...
            return _createClass(Bone, [{
                ...
                value: function() {
                    ...
                    if (this.rotation = this.transform.skX, i) t = this.resultTransform.getMatrix(),
                        e.Matrix.mul(t, i, this.resultMatrix), this.resultRotation = this.rotation;
                    else if (this.resultRotation = this.rotation + this.parentBone.resultRotation,
                        this.parentBone)
                        if (this.inheritRotation && this.inheritScale) t = this.resultTransform.getMatrix(),
                            e.Matrix.mul(t, this.parentBone.resultMatrix, this.resultMatrix);
                        else {
                        ...
                        switch (this._diyTexture && (n = this._diyTexture), this.currDisplayData.type) {
                            ...
                            case 1:
                                ...
                                if (null == this.currDisplayData.bones) {
                                    ...
                                    if (this._parentMatrix)
                                        if (y) e.Matrix.mul(y, this._parentMatrix, e.Matrix.TEMP),
            ...
            return _inherits(Skeleton, e.Sprite), _createClass(Skeleton, [{
                ...
                value: function() {
                    ...
                    for (f = 0, m = this._boneList.length; f < m; f++)
                        if (c = this._boneList[f], o = this._bindBoneBoneSlotDic[c.name],
                            c.resultMatrix.copyTo(this._boneMatrixArray[f]), o)
                    ...
                    if (this._drawOrder)
                        for (f = 0, g = this._drawOrder.length; f < g; f++) F = k[(l = this._boneSlotArray[this._drawOrder[f]]).name],
                ...
                value: function() {
                    ...
                    for (i = 0; i < dt; i++) {
                        ...
                        for (var mt = 0; mt < ft; mt++)
                            for ((lt = new v()).skinName = o.getUTFString(),
                                yt[lt.skinName] = lt, at = o.getInt16(), a = 0; a < at; a++)
                                for (ot = new x(),
                                    lt.deformSlotDataList.push(ot), rt = o.getInt16(), r = 0; r < rt; r++)
                                    for (ut = new D(),
                        ...
                        for ((Pt = new P()).name = Vt[Yt++], Bt = o.getUint8(), a = 0; a < Bt; a++) {
                            for ((Ft = new w()).name = Vt[Yt++], St = this.boneSlotDic[Ft.name], Lt = o.getUint8(),
                                ...
                                if (wt = new F(), this.skinSlotDisplayDataArr.push(wt), wt.name = Vt[Yt++], wt.attachmentName = Vt[Yt++],
                                    ...
                                    wt.height = o.getFloat32(), wt.type = o.getUint8(), wt.verLen = o.getUint16(), (X = o.getUint16()) > 0)
                                    for (wt.bones = [],
                                        s = 0; s < X; s++) {
                                        var Xt = o.getUint16();
                                        wt.bones.push(Xt);
