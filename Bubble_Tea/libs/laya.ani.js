! function(t, e) {
    ...
    class u {
        ...
        static getBezierRate(t, e, i, a, r) {
            ...
            for (l = o.length, h = 0; h < l; h += 2)
                if (t <= o[h]) return u._bezierResultCache[n] = o[h + 1],
                    o[h + 1];
    ...
    class F extends e.Sprite {
        ...
        _createGraphics(t = -1) {
            ...
            for (m = 0, y = this._boneList.length; m < y; m++)
                if (_ = this._boneList[m], o = this._bindBoneBoneSlotDic[_.name],
                    _.resultMatrix.copyTo(this._boneMatrixArray[m]), o)
            ...
            if (this._drawOrder)
                for (m = 0, g = this._drawOrder.length; m < g; m++) k = I[(l = this._boneSlotArray[this._drawOrder[m]]).name],
    ...
    class U extends _ {
        ...
        parse(t) {
            super.parse(t), this.event(e.Event.LOADED, this), this._aniVersion === U.LAYA_ANIMATION_VISION ? this._isParseAudio = !0 : (this._aniVersion,
                U.LAYA_ANIMATION_160_VISION), this._mainTexture ? this._parsePublicExtData() : this._parseTexturePath();
        }
        _parseTexturePath() {
            if (this._isDestroyed) this.destroy();
            else {
            ...
            for (i = 0; i < ct; i++) {
                ...
                for (var yt = 0; yt < mt; yt++)
                    for ((lt = new f()).skinName = o.getUTFString(),
                        xt[lt.skinName] = lt, at = o.getInt16(), a = 0; a < at; a++)
                        for (ot = new M(),
